...
<script setup lang="ts">
import { SForm } from '@antdvr/library-3.x' // [!code highlight]
import { formGroupsDefiner } from '@antdvr/library-3.x' // [!code highlight]
...
::: details ESlint => `@antdvr/eslint-config-3.x`
...
import antdvr from "@antdvr/eslint-config-3.x"; // [!code highlight]
...
::: details TypeScript => `@antdvr/library-3.x/globals`
...
```json
{
  "compilerOptions": {
    ...
    "types": ["@antdvr/library-3.x/globals"] // [!code highlight]
  }
}
```
...
::: details unplugin-auto-import => `@antdvr/library-3.x/resolver` (only v4.x+)
...
::: details `@antdvr/library-3.x` (only v4.x+) {open}
